...
Real cgrad(Vector &W, vector<TImg> const &train, Args const &args) {
...
int main(int argc, char *argv[]) {
    if (argc != 2)
        return printfln("Usage: %s inputfile", argv[0]), 0;
    ...
    auto phi = [](Real x, int n) -> Real { return n == 1 ? 1. : x / 4.; };

    auto size = 1 + N;
    printfln("Vector size = %d", size);

    auto setup = [&](vector<TImg> &set, vector<MNISTData> const &data) {
        for (auto &img : data) {
            auto l = img.label;
            auto y = (l == L) ? +1 : -1;
            auto v = Vector(size);
    ...
    auto evaluate = [&](vector<TImg> const &set) {
        ...
        Cnl /= T;
        auto ninc = T - ncor;
        printfln("Percent correct = %.4f%%, #correct = %d/%d, #incorrect = %d/%d", ncor * 100. / T, ncor, T, ninc, T);
        auto Cl = lambda * (V * V);
    ...
    for (auto j : range(2 + N))
        links.at(j) = Index(format("l%d", j), M, Link);
