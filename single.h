...
ITensor polarU(ITensor const &T, std::initializer_list<Index> inds) {
    ITensor F{IndexSet(inds)}, S, G;
    svd(T, F, S, G, {"Truncate", false});
    auto fi = commonIndex(F, S), gi = commonIndex(G, S);
    F *= delta(fi, gi);
    return F * G;
}
...
MPS *getTrainState(MPSArr &trainmps, TState const &t) { return &(trainmps.at(t.l).at(t.n)); }

MPS const *getTrainState(MPSArr const &trainmps, TState const &t) { return &(trainmps.at(t.l).at(t.n)); }

ITensor mult(int lc, ITensor B, TState const &t, MPSArr const &trainmps) {
    auto rc = lc + 3;
...
ITensor mult(int lc, Real f, TState const &t, MPSArr const &trainmps) {
    auto rc = lc + 3;
    auto &tmps = *getTrainState(trainmps, t);
    auto N = tmps.N();
    ITensor v;
...
Real quadcost(ITensor B, vector<TState> const &ts, MPSArr const &trainmps, int L, ParallelDo const &parallel_do,
...
//
void exact(ITensor &B, vector<TState> &ts, ParallelDo const &parallel_do, Args const &args) {
    auto lambda = args.getReal("lambda", 0.);
...
// Conjugate gradient
//
void cgrad(ITensor &B, vector<TState> &ts, MPSArr const &trainmps, ParallelDo const &parallel_do, Args const &args) {
    auto Ntrain = ts.size();
    auto L = args.getInt("Label");
...
void fast_cgrad(ITensor &B, vector<TState> &ts, MPSArr const &trainmps, ParallelDo const &parallel_do,
                Args const &args) {
    auto Ntrain = ts.size();
    auto L = args.getInt("Label");
    auto Npass = args.getInt("Npass");
...
//
void pinv(ITensor &B, vector<TState> &ts, ParallelDo const &parallel_do, Args const &args) {
...
// M.L. DMRG
//
void mldmrg(MPS &W, MPSArr &trainmps, vector<TState> &ts, Sweeps const &sweeps, ParallelDo const &parallel_do,
    ...
    for (auto sw : range1(sweeps)) {
        ...
        for (int b = 1, ha = 1; ha <= 2; sweepnext(b, ha, N)) {
            ...
            else if (method == "fast_conj")
                fast_cgrad(B, ts, trainmps, parallel_do, args);
            else if (method == "exact")
                exact(B, ts, parallel_do, args);
            else if (method == "pinv") {
                auto BB = B;
