...
int main(int argc, char *argv[]) {
    ...
    for (auto &t : ts) {
        ...
        while (t.n == -1) {
            ...
            t.n = nextTrainN(t.l);
            if (++count > 20)
                Error("Infinite loop while setting up ts");
        }
    }
    ...
    auto args = Args{"Label", L,     "lambda",  lambda,  "Wname",      Wname,       "Method",  method,
                     "Npass", Npass, "Ntarget", Ntarget, "PCut",       pcut,        "alpha",   alpha,
                     "clip",  clip,  "cconv",   cconv,   "PauseSteps", pause_steps, "Precalc", precalc};
