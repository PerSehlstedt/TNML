...
template <size_t nlabel> void fullTest(vector<MPS> const &Ws, MPSArr const &set_test, array<long, nlabel> labels) {
    long NL = Ws.size();
    ...
    printfln("%d/%d correct (%.2f%%), %d/%d incorrect (%.2f%%)", tncor, ntest, tncor * 100. / ntest, tninc, ntest,
             tninc * 100. / ntest);
    auto tot_counts = 0l;
    for (auto l : range(10)) {
        auto nt = counts[l];
