...
class ParallelDo {
  ...
  public:
    ...
    ParallelDo(vector<Bound> const &bs) : bounds_(bs) {}

    ParallelDo(int Nthread, int Ntask) : bounds_(Nthread) {
    ...
    size_t Nthread() const { return bounds_.size(); }

    vector<Bound> const &bounds() const { return bounds_; }

    template <typename Task> void operator()(Task &&T) const {
        const auto Nf = 16ul;
        if (bounds_.size() > Nf)
            Error("Need to increase size of futs");
